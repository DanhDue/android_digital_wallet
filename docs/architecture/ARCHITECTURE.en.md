...
`commons.android-feature` convention plugin. When generated, install-time features also include
an isolated `:sample` application module at `features/{name}/sample/` applying `commons.android-sample`.

```text
features/{name}/
├── sample/                  🔴  standalone — Isolated Sandbox runner (:features:{name}:sample)
│   ├── src/main/
│   │   ├── AndroidManifest.xml   (Launchable activity declaration)
│   │   ├── res/values/strings.xml
│   │   └── kotlin/com/{org}/features/{name}/sample/
│   │       ├── {Name}SampleApp.kt        (@HiltAndroidApp runner entry)
│   │       ├── {Name}SampleActivity.kt   (Standalone Compose window loading {Name}Route)
...
| `:features:*:sample` | `com.danhdue.features.{feature}.sample` | Standalone sandbox runner APK application. Configured via `commons.android-sample`. Enables local hot-reload and isolated debugging without assembling `:app`. | Target `:features:*`, `:packages:platform`, `:libraries:testutils` |
...
flowchart LR
    %% ==========================================
    %% 1. LEFT: Architecture & Contract Gate
    %% ==========================================
    subgraph GATE["🛡️ ARCHITECTURE & CONTRACT GATE"]
        direction TB
        ...
        BCV["<b>Binary Compatibility Validator</b><br/><i>ABI Contract Gate (apiCheck)</i><br/>───────────────<br/>• Track public API signatures (.api)<br/>• Lock down 5 shared packages<br/>• Prevent breaking runtime ABI<br/><i>(Gradle apiCheck · CI Phase 6)</i>"]
    end
    ...
    subgraph Features["🧩 FEATURES (:features:* — Blind to each other)"]
        ...
        F_SAMPLE["<b>:features:settings:sample</b><br/><i>(Standalone Sandbox APK)</i>"]
        F_SCAN["<b>:features:scanner</b><br/><i>(DFM on-demand)</i>"]
        F_BIZ["<b>:features:…</b><br/><i>(Extended features)</i>"]
        F_SAMPLE -.->|"Isolated Runner"| F_SET
    end
    ...
    BCV -.->|"Lock public ABI"| Packages
    BCV -.->|"Lock public seam"| Seam
...
### 4.2 Super App Governance: 4 Core Pillars & 8 Criteria

To operate as an enterprise-grade Super App platform, the architecture strictly enforces **4 Core Pillars** across **8 Governance Criteria**:

| Pillar | Criterion | Requirement & Enforcement | Status |
|---|---|---|:---:|
| **1. Container & Modules** | **1.1 Host App (App Vỏ)** | Thin composition container (`:app` & `:shell`) hosting shared foundational capabilities (Auth, Network, Storage, Navigation). Contains zero feature business logic. | **100%** ✅ |
| | **1.2 Mini Apps (App Ruột)** | Every capability isolated into independent `:features:*` modules. Supports on-demand **Dynamic Feature Modules (DFM)** (e.g. `:features:scanner`) downloaded runtime via `SplitInstallManager` + Java `ServiceLoader`. | **100%** ✅ |
| **2. Centralized Comms & Routing** | **2.1 DeepLink Router Engine** | Mini Apps are 100% blind to one another. Zero cross-feature imports. All navigation is routed via pure JVM `DeepLinkParser` & `DeepLinkRouter` using URL schemes (`myapp://`) or App Links (`https://`). Enforced by Konsist **K1** & **K9**. | **100%** ✅ |
| | **2.2 Event Bridge** | Cross-boundary communication relies on stateless `AppEventBus` (`SharedFlow<AppEvent>`) in `:packages:platform`. Features emit decoupled events without knowing consumers. | **100%** ✅ |
| **3. State Isolation** | **3.1 Local State Packaging** | Eliminates fragile global state. Each Mini App manages its own state lifecycle via isolated `MviViewModel<State, Action, Event>`. Enforced by Konsist **K5**. | **100%** ✅ |
| | **3.2 Tiered Dependency Injection** | Strict inversion of control. Host provides singletons; implementations in `data` layers are strictly `internal` (Konsist **K4**). Mini Apps only expose clean `domain` interfaces and `presentation` screens. | **100%** ✅ |
| **4. Lifecycle Governance & CI/CD** | **4.1 Sandbox Development** | Each Mini App is a self-contained testbed with a dedicated `:sample` APK runner (`commons.android-sample`). Developers build, run, and test features without compiling the full Super App. | **100%** ✅ |
| | **4.2 Internal API Contract Governance** | Public ABI of the 5 shared foundation packages (`core`, `platform`, `network`, `framework`, `ui_kit`) is tracked by **Binary Compatibility Validator (BCV)**. Breaking changes immediately fail `./gradlew apiCheck` in CI. | **100%** ✅ |

### 4.3 Sandbox Development: Standalone Mini App Runners

To maximize developer velocity in large multi-team organizations, each feature includes an isolated `:sample` application module:
- **Convention Plugin (`commons.android-sample`)**:
  - Located at `buildSrc/src/main/kotlin/commons/AndroidSampleConventionPlugin.kt`.
  - Configures `com.android.application`, Hilt, Compose, and quality tools.
  - Automatically computes an isolated `applicationId`:
    ```kotlin
    ...
    ```
  - Wires the target feature as the sole implementation dependency.
- **Reference Runner (`:features:settings:sample`)**:
  - `SettingsSampleApp`: `@HiltAndroidApp` runner application.
  - `SettingsSampleActivity`: Standalone ComponentActivity hosting `SettingsRoot`.
  - `SettingsSampleModule`: Testbed DI bindings providing mock navigation and isolated state observation.
- **Workflow**:
  ```bash
...
### 4.4 Binary Compatibility Validator (BCV) & ABI Contract Governance

Shared package ABIs are treated as immutable protocol contracts:
- **Plugin Integration**: `org.jetbrains.kotlinx.binary-compatibility-validator:0.17.0` configured in root `build.gradle.kts`.
- **Target Scope**: Strictly protects the 5 shared foundation packages:
...
- **Non-Public Markers & Exclusions**:
  - Package `hilt_aggregated_deps` ignored to prevent internal KSP generation artifacts from polluting contracts.
  - `@InternalApi` annotation in `:packages:core` marks internal declarations exempt from public ABI checks.
- **CI / Acceptance Verification**:
  - `./gradlew apiCheck`: Compares compiled bytecode against checked-in `.api` snapshots. Integrated into root `:check` and Phase 6 of `./scripts/acceptance_check.sh`.
  - `./gradlew apiDump`: Regenerates `.api` signatures when an intentional, backward-compatible addition is introduced.
...
| Rule | Checks | Status |
|---|---|---|
| K1 | a feature does not import another feature (unless whitelisted) | **enforced** (whitelist empty) |
| K2 | `..presentation..` ⇏ `..data..`; `..domain..` ⇏ `..presentation..` / `..data..` | **enforced** |
| K3 | `..domain..` has no `android.*` / `androidx.*` import | **enforced** |
| K4 | top-level declarations under `..features..data..` are `internal` | **enforced** |
...
| K6 | only `:app` / `:shell` aggregate more than one feature | **enforced** (whitelist empty) |
...
| K9 | a `*Route : NavKey` used cross-feature is declared in `:platform`, not in a feature | **enforced** |
...
| **Quality Gates** | **Konsist** (architecture), **Binary Compatibility Validator (BCV)** (ABI contract check), detekt, Spotless, JaCoCo | Layer / boundary / naming enforcement, public ABI stability, static analysis, formatting, coverage |
...
| **Build** | Gradle Kotlin DSL, `buildSrc` convention plugins (`commons.android-library` / `-compose` / `-feature` / `-sample` / `dagger-hilt`), `Versions.kt` + `Deps.kt` | Centralized versions (no hardcoding), per-module conventions, standalone sandbox runner configuration |
...
- [`.devtool/epic/sandbox_and_contract_governance/sandbox_and_contract_governance.en.md`](../../.devtool/epic/sandbox_and_contract_governance/sandbox_and_contract_governance.en.md) — Sandbox Development & BCV Contract Governance epic overview and acceptance matrix.
...
- `packages/*/api/*.api` — Binary Compatibility Validator checked-in public ABI declarations.
...
| **Sandbox Testing** | Standalone Mini App runner APKs (`:features:*:sample`) via `commons.android-sample` convention plugin |
| **Contract Governance** | Public ABI locked via JetBrains **Binary Compatibility Validator (BCV 0.17.0)** |
| **Code generation** | KSP (Hilt / Moshi / Room); Mason for features |
| **Enforcement** | Konsist K1–K10 + BCV `apiCheck` + a Gradle guard + CI (`acceptance_check.sh` Phases 0–6) |
...
9. **Isolated Sandbox Development** — Mini Apps develop and run independently via `:sample` APK testbeds.
10. **Strict Contract Protection** — public ABI breaches fail builds before PR merge.

### 3. Quick commands

```bash
# Create a new feature (with standalone :sample runner)
...
# Run Mini App standalone sandbox
./gradlew :features:settings:sample:assembleDebug

# Check Binary Compatibility & Contract signatures
./gradlew apiCheck

# Dump intentional API additions to .api snapshots
./gradlew apiDump

# Full local gate (matches CI)
./gradlew :konsist-test:test detekt spotlessCheck apiCheck testDebugUnitTest assembleDebug

# Run full end-to-end acceptance suite (Phases 0–6)
...
**This architecture ensures: Scalability • Testability • Maintainability • Consistency • Contract Governance.**
