...
@Inject
constructor(private val remoteDataSource: AuthenticationRemoteDataSource) : LoginRepository {
    override suspend fun login(email: String, password: String): DataState<Login> {
        val request =
        ...
        return when (val response = remoteDataSource.login(request)) {
            is com.danhdue.framework.network.calladapter.NetworkResponse.Success -> {
                com.danhdue.framework.network.DataState.Success(response.body.toDomain())
            }
            is com.danhdue.framework.network.calladapter.NetworkResponse.ApiError -> {
                com.danhdue.framework.network.DataState.Error(Exception(response.body.toString()))
            }
            is com.danhdue.framework.network.calladapter.NetworkResponse.NetworkError -> {
                com.danhdue.framework.network.DataState.Error(response.error)
            }
