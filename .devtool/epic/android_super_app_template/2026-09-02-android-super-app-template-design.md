...
3. **Gom `FeatureEntry` của DFM qua `ServiceLoader`** — file `META-INF/services/...FeatureEntry` **do `:app` sở hữu** (`app/src/main/resources/...`), mỗi DFM **append 1 dòng FQCN** `<Name>FeatureEntry` vào đó (`mvi_feature --delivery on-demand` thêm, `remove_feature` gỡ). Bundletool cấm 2 feature module cùng root-resource path khác nội dung (`InvalidBundleException` khi `bundleDebug`), nên KHÔNG để file này một-bản-mỗi-module — phải aggregate ở base `:app`. Sau `SplitInstallManager` cài xong + `SplitCompat.install()`, `:shell` `ServiceLoader.load(FeatureEntry::class.java)` → duyệt **từng phần tử, bỏ qua** entry của split chưa cài (`ServiceConfigurationError`) → lấy `EntryProviderInstaller` của các split đã cài → cộng vào tập installer đang cấp cho `NavDisplay`.
