...
- **G9 — Sandbox development.** Every feature builds/tests standalone (`:features:x:testDebugUnitTest` with no `:app`). Per-feature UI runners are now fully completed via the `sandbox_and_contract_governance` epic extension through convention plugin `commons.android-sample` and `:features:*:sample` runners.
...
- **G11 — Contract Governance (BCV).** Safeguard the public ABI across all 5 foundation packages (`core`, `platform`, `network`, `framework`, `ui_kit`) using JetBrains Binary Compatibility Validator 0.17.0 (`apiCheck` in CI Phase 6, `apiDump`).
- **G12 — Pure JVM Deep Link Router.** URL Scheme (`myapp://`) & App Links (`https://`) routing engine decoupled from Android framework dependencies, resolving cross-feature navigation safely.
...
- **Formal artifact publishing / SemVer registry.** Monorepo builds compile together; ABI stability is governed directly by BCV `apiCheck` in CI Phase 6, making external artifact publishing unnecessary.
...
- [Task 12: Migrate remaining feature cross-imports → empty whitelist](../../features/done/task_12_migrate_remaining_features.md)
