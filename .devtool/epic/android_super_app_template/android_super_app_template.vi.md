...
- **G9 — Sandbox development.** Mỗi feature build/test độc lập (`:features:x:testDebugUnitTest` không cần `:app`). Runner UI độc lập từng feature nay đã hoàn thành trọn vẹn ở epic tiếp nối `sandbox_and_contract_governance` thông qua convention plugin `commons.android-sample` và runner `:features:*:sample`.
- **G10 — DFM-ready + 1 pilot on-demand.** Hợp đồng cross-feature được thiết kế để feature bất kỳ chuyển sang `com.android.dynamic-feature` **không đổi cơ chế nav của host** — chỉ đổi đường resolve entry. Chuyển **`scanner`** thành Dynamic Feature Module tải theo yêu cầu làm ví dụ mẫu (`FeatureEntry` + `ServiceLoader` + `SplitInstallManager`); `home`/`settings` giữ install-time.
- **G11 — Contract Governance (BCV).** Bảo vệ public ABI của 5 packages nền tảng (`core`, `platform`, `network`, `framework`, `ui_kit`) bằng JetBrains Binary Compatibility Validator 0.17.0 (`apiCheck` trong CI Phase 6, `apiDump`).
- **G12 — Pure JVM Deep Link Router.** Engine định tuyến URL Schema (`myapp://`) & App Links (`https://`) không phụ thuộc Android, giải quyết cross-feature navigation an toàn.
...
- **Publish artifact độc lập / SemVer registry.** Monorepo build cùng nhau; sự ổn định ABI được kiểm soát trực tiếp bởi BCV `apiCheck` ở CI Phase 6, không cần hệ thống phân phối package ngoài.
...
- [Task 4: Cập nhật Mason brick cho wiring mới](../../features/done/task_4_mason_brick_wiring.md)

### Phase 1 — Tách god-module
- [Task 5: Bóc `:core`](../../features/done/task_5_extract_core_module.md)
...
- [Task 16: CI `bundleDebug` + kiểm thử nghiệm thu end-to-end](../../features/done/task_16_e2e_acceptance_validation.md)
